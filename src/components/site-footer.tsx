    ...
    <footer className="bg-[color:var(--brand-dark)] text-white pt-16 sm:pt-20 pb-8 px-6 md:px-10">
      <div className="max-w-6xl mx-auto grid gap-12 md:grid-cols-4">
        <div className="md:col-span-1">
          <div className="text-2xl font-display font-bold tracking-tight mb-3">{brand.brand_name}</div>
          <p className="text-[10px] uppercase tracking-[0.3em] text-[color:var(--brand-accent)] mb-5">{brand.tagline}</p>
          <p className="text-white/40 text-sm font-light leading-relaxed">
            Bespoke sofas hand-tailored in Central India. Serving the discerning homes of {brand.cities} since {brand.established}.
          </p>
        </div>
        <div>
        ...
        <div>
          ...
          <ul className="space-y-3 text-sm text-white/60 font-light">
            ...
            <li><a href={`https://wa.me/${brand.whatsapp}`} className="hover:text-white transition-colors">WhatsApp · {brand.phone}</a></li>
      ...
      <div className="max-w-6xl mx-auto mt-14 pt-8 border-t border-white/5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 text-[10px] font-bold uppercase tracking-widest text-white/30">
        <span>© {new Date().getFullYear()} {brand.brand_name} · Hand-tailored in {brand.cities}</span>
